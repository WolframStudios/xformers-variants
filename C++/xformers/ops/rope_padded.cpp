...
/*
Original Python source: xformers/ops/rope_padded.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
std::any rope_padded(torch::Tensor xq, torch::Tensor xk, torch::Tensor xv, torch::Tensor cache_k, torch::Tensor cache_v, BlockDiagonalCausalWithOffsetPaddedKeysMask attn_bias) {
  ...
  Arguments:
      ...
      internal_dtype: set to "f32" or "f64" to enforce dtype in the calculation
  */
  return translation::unsupported<std::any>("Translated placeholder for xformers/ops/rope_padded.py::rope_padded");
}
