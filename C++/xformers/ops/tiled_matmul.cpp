...
/*
Original Python source: xformers/ops/tiled_matmul.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
bool _should_use_triton(torch.device device, torch.dtype dtype) {
...
void tiled_matmul_setup_context(std::any ctx, std::any inputs, torch::Tensor output) {
  translation::unsupported_void("Translated placeholder for xformers/ops/tiled_matmul.py::tiled_matmul_setup_context");
}

std::any tiled_matmul_bwd(std::any ctx, std::any flat_grad_c) {
