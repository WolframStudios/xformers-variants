...
/*
Original Python source: xformers/ops/sequence_parallel_fused_ops.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
std::any _is_fp8_dtype(torch.dtype dt) {
  return translation::unsupported<std::any>("Translated placeholder for xformers/ops/sequence_parallel_fused_ops.py::_is_fp8_dtype");
}
...
class _FusedSequenceParallel {
 public:
  ...
  https://github.com/fairinternal/xformers/pull/504.
  */
  _FusedSequenceParallel() = default;
  void __init__(torch.device device, dist.ProcessGroup group) {
    translation::unsupported_void("Translated placeholder for xformers/ops/sequence_parallel_fused_ops.py::__init__");
  }
  ...
  void linear_and_reducescatter(std::function<std::any(std::vector<std::any>)>[[std::vector<torch::Tensor], int64_t, std::function<std::any(std::vector<std::any>)>[[], torch.cuda.Stream]], void> my_matmul, std::vector<torch::Tensor> gathered_outputs, std::vector<torch::Tensor> scattered_outputs, int64_t timeout_s, bool _wait = true, bool _memcpy = true) {
    ...
    Perform a fused linear layer followed by a reduce-scatter
    */
    translation::unsupported_void("Translated placeholder for xformers/ops/sequence_parallel_fused_ops.py::linear_and_reducescatter");
  }
};

bool _can_ranks_communicate_all_to_all_over_nvlink(dist.ProcessGroup group) {
  return translation::unsupported<bool>("Translated placeholder for xformers/ops/sequence_parallel_fused_ops.py::_can_ranks_communicate_all_to_all_over_nvlink");
}

std::optional<_FusedSequenceParallel> _lazy_init(torch.device device, dist.ProcessGroup group) {
  return translation::unsupported<std::optional<_FusedSequenceParallel>>("Translated placeholder for xformers/ops/sequence_parallel_fused_ops.py::_lazy_init");
}

torch.cuda.Stream _default_stream_factory() {
