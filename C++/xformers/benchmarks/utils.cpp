...
/*
Original Python source: xformers/benchmarks/utils.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
class NotSupportedInputError : public Exception {
 public:
  NotSupportedInputError() = default;
};

std::any get_func_name(std::any fn) {
  return translation::unsupported<std::any>("Translated placeholder for xformers/benchmarks/utils.py::get_func_name");
}

void pretty_print(std::any results, std::any title, std::any units) {
  /*
  Printout the contents of a dict as a human-readable and Markdown compatible array
  */
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::pretty_print");
}

void pretty_plot(std::any results, std::any title, std::string units, std::any filename, std::any dash_key = "", std::any legend_loc = "lower right") {
  /*
  Graph out the contents of a dict.
  Dash key means that if the result label has this key, then it will be displayed with a dash
  */
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::pretty_plot");
}

void pretty_barplot(std::any results, std::any title, std::string units, std::any filename, std::any dash_key = "") {
  /*
  Graph out the contents of a dict.
  Dash key means that if the result label has this key, then it will be displayed with a dash
  */
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::pretty_barplot");
}

void rmf(std::string filename) {
  /*
  Remove a file like rm -f.
  */
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::rmf");
}

Generator temp_files_ctx(int64_t num) {
  /*
  A context to get tempfiles and ensure they are cleaned up.
  */
  return translation::unsupported<Generator>("Translated placeholder for xformers/benchmarks/utils.py::temp_files_ctx");
}

std::vector<std::tuple<std::unordered_map<std::string, std::any>, std::any>> _benchmark_results_from_csv(std::string filename) {
  return translation::unsupported<std::vector<std::tuple<std::unordered_map<std::string, std::any>, std::any>>>("Translated placeholder for xformers/benchmarks/utils.py::_benchmark_results_from_csv");
}

void _benchmark_results_to_csv(std::string filename, std::vector<std::tuple<std::unordered_map<std::string, std::any>, std::any>> results) {
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::_benchmark_results_to_csv");
}

std::vector<std::any> _finalize_results(std::vector<std::tuple<std::unordered_map<std::string, std::any>, std::any>> results) {
  /*
  Returns a `benchmark.Compare` object, except that if we have runs
  with different algorithms, we also add the algorithm name
  in the column titles
  */
  return translation::unsupported<std::vector<std::any>>("Translated placeholder for xformers/benchmarks/utils.py::_finalize_results");
}

void _render_bar_plot(std::vector<std::any> results, std::string store_results_folder) {
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::_render_bar_plot");
}

argparse.ArgumentParser create_argparser() {
  /*
  Create CLI argument parser.
  */
  return translation::unsupported<argparse.ArgumentParser>("Translated placeholder for xformers/benchmarks/utils.py::create_argparser");
}

void benchmark_main_helper(std::any benchmark_fn, std::vector<std::unordered_map<std::string, std::any>> cases, std::any arg_parser, std::unordered_map<std::string, std::any> kwargs = {}) {
  /*
  Helper function to run benchmarks.
  Supports loading previous results for comparison, and saving current results to file.
  */
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::benchmark_main_helper");
}

void benchmark_run_and_compare(std::any benchmark_fn, std::vector<std::unordered_map<std::string, std::any>> cases, std::vector<std::string> compare, bool omit_baselines = false, bool fail_if_regression = false, bool quiet = false, std::string optimized_label = "optimized") {
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::benchmark_run_and_compare");
}

std::any _is_oom_error(std::any e) {
  return translation::unsupported<std::any>("Translated placeholder for xformers/benchmarks/utils.py::_is_oom_error");
}

void _fail_if_regressions(std::vector<std::any> results, std::vector<std::any> reference, double atol_s, double rtol) {
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::_fail_if_regressions");
}

void benchmark_main_helper2(std::string name, std::any functions, bool fw = false, bool bw = false, bool cuda_graph = true, std::unordered_map<std::string, std::any> kwargs = {}) {
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::benchmark_main_helper2");
}

void product_dict(std::unordered_map<std::string, std::any> kwargs = {}) {
  translation::unsupported_void("Translated placeholder for xformers/benchmarks/utils.py::product_dict");
}
