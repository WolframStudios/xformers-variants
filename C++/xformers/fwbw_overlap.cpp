...
/*
Original Python source: xformers/fwbw_overlap.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
class EventOverlapHolder : public Tensor {
 public:
  /*
  Holds a CUDAEvent. Why does it need to be a tensor?
  So that its `gradient` can also hold a CUDAEvent for
  ...
  */
  ...
  std::any __tensor_flatten__() {
    return translation::unsupported<std::any>("Translated placeholder for xformers/fwbw_overlap.py::__tensor_flatten__");
  }
  std::string __repr__() {
    return translation::unsupported<std::string>("Translated placeholder for xformers/fwbw_overlap.py::__repr__");
  }
  void current_stream_wait() {
    translation::unsupported_void("Translated placeholder for xformers/fwbw_overlap.py::current_stream_wait");
  }
  std::any __torch_dispatch__(std::any func, std::any types, std::any args, std::any kwargs) {
    return translation::unsupported<std::any>("Translated placeholder for xformers/fwbw_overlap.py::__torch_dispatch__");
  }
};

class _ExitCompute : public Function {
 public:
  /*
  Execution order:
  [compute]
...
class _EnterCompute : public Function {
 public:
  /*
  Execution order:
  [comms]
...
class _FillGradientForOverlapHolder : public Function {
 public:
  /*
  If the OverlapHolder is not used (eg with 1 GPU) then it does
  not get any gradient, and the BW pass is stuck. This ensures it does
...
struct PhaseBoundary {
 public:
  ...
  void __post_init__() {
    translation::unsupported_void("Translated placeholder for xformers/fwbw_overlap.py::__post_init__");
  }
  std::string __str__() {
    return translation::unsupported<std::string>("Translated placeholder for xformers/fwbw_overlap.py::__str__");
  }
  void __call__() {
    translation::unsupported_void("Translated placeholder for xformers/fwbw_overlap.py::__call__");
  }
};
...
class _GlobalAutogradThread {
 public:
  _GlobalAutogradThread() = default;
  void run() {
    translation::unsupported_void("Translated placeholder for xformers/fwbw_overlap.py::run");
  }
  void cleanup_at_exit() {
    translation::unsupported_void("Translated placeholder for xformers/fwbw_overlap.py::cleanup_at_exit");
  }
};
...
class _CurrentForwardState {
 public:
  _CurrentForwardState() = default;
};

void before_forward(bool record_fw_chunks) {
  /*
  call this before entering a new FW pass
  */
...
bool flush_single_bw_chunk() {
  return translation::unsupported<bool>("Translated placeholder for xformers/fwbw_overlap.py::flush_single_bw_chunk");
}

void flush_pending_bw() {
  /*
  Flush all BW chunks that we can compute recursively
  */
