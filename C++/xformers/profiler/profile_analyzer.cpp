...
/*
Original Python source: xformers/profiler/profile_analyzer.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
class FakeKinetoEvent {
 public:
  FakeKinetoEvent() = default;
  void __init__(torch._C._autograd._KinetoEvent e) {
    translation::unsupported_void("Translated placeholder for xformers/profiler/profile_analyzer.py::__init__");
  }
};

int64_t _attention_flops(std::any queries, std::any values, bool causal, std::string fmt = "BHMK") {
  return translation::unsupported<int64_t>("Translated placeholder for xformers/profiler/profile_analyzer.py::_attention_flops");
}

int64_t _get_arg_idx(std::any op, std::vector<std::any> varargs = {}) {
  return translation::unsupported<int64_t>("Translated placeholder for xformers/profiler/profile_analyzer.py::_get_arg_idx");
}

torch._C._autograd._KinetoEvent _replace_if_needed(torch._C._autograd._KinetoEvent e) {
  /*
  Adds a flops amount for operators that don't have this information in Kineto already
  This mostly applies for the attention for now, as GEMMs are already calculated by Kineto
  and other operations are negligible.
  */
  return translation::unsupported<torch._C._autograd._KinetoEvent>("Translated placeholder for xformers/profiler/profile_analyzer.py::_replace_if_needed");
}

struct AnalyzedTrace {
 public:
  std::unordered_map<torch.dtype, double> operations_per_dtype_fw;
  std::unordered_map<torch.dtype, double> operations_per_dtype_bw;
  double total_time_s;
  AnalyzedTrace() = default;
  double compute_num_ops(torch.dtype dtype, bool fw = true, bool bw = true) {
    return translation::unsupported<double>("Translated placeholder for xformers/profiler/profile_analyzer.py::compute_num_ops");
  }
  double compute_hfu(std::unordered_map<torch.dtype, double> hardware_flops) {
    return translation::unsupported<double>("Translated placeholder for xformers/profiler/profile_analyzer.py::compute_hfu");
  }
  double compute_mfu(std::unordered_map<torch.dtype, double> hardware_flops) {
    return translation::unsupported<double>("Translated placeholder for xformers/profiler/profile_analyzer.py::compute_mfu");
  }
  Sequence[torch._C._autograd._KinetoEvent] _find_all_root_events_with_flops(Sequence[torch._C._autograd._KinetoEvent] all_events) {
    return translation::unsupported<Sequence[torch._C._autograd._KinetoEvent]>("Translated placeholder for xformers/profiler/profile_analyzer.py::_find_all_root_events_with_flops");
  }
  'AnalyzedTrace' from_profile(Sequence[torch._C._autograd._KinetoEvent] events) {
    return translation::unsupported<'AnalyzedTrace'>("Translated placeholder for xformers/profiler/profile_analyzer.py::from_profile");
  }
};
