...
/*
Original Python source: xformers/profiler/find_slowest.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
void print_json_as_dataframe(std::any json_list) {
  translation::unsupported_void("Translated placeholder for xformers/profiler/find_slowest.py::print_json_as_dataframe");
}

std::any compute_std_dev_of_event_durations_over_ranks(std::any events, std::any top = 5) {
  return translation::unsupported<std::any>("Translated placeholder for xformers/profiler/find_slowest.py::compute_std_dev_of_event_durations_over_ranks");
}

std::vector<std::unordered_map<std::string, std::string>> sort_nccl_events(std::any nccl_events, int64_t top_k = 3, int64_t last_k = 3) {
  return translation::unsupported<std::vector<std::unordered_map<std::string, std::string>>>("Translated placeholder for xformers/profiler/find_slowest.py::sort_nccl_events");
}

pd.DataFrame read_one_file(std::string profile_trace_path) {
  return translation::unsupported<pd.DataFrame>("Translated placeholder for xformers/profiler/find_slowest.py::read_one_file");
}

tuple[pd.DataFrame, pd.DataFrame] parse_one_file(std::string profile_trace_path) {
  return translation::unsupported<tuple[pd.DataFrame, pd.DataFrame]>("Translated placeholder for xformers/profiler/find_slowest.py::parse_one_file");
}

void print_profiling_info(std::string cuda_profile_dir) {
  translation::unsupported_void("Translated placeholder for xformers/profiler/find_slowest.py::print_profiling_info");
}
