...
/*
Original Python source: xformers/profiler/api.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
std::any profile(std::string output_dir, std::optional<nn.Module> module = std::nullopt, Sequence[std::tuple<std::any, int64_t, int64_t]> schedule) {
  /*
  A pre-configured profiler that will run on the first ~20 steps of the training
  It will provide multiple traces that can be exploited later.
  Use it in a context manager around your training loop, and call `xformers.profiler.step`
  ...
  .. code-block:: python
      ...
      xprofiler.stop()
  */
  return translation::unsupported<std::any>("Translated placeholder for xformers/profiler/api.py::profile");
}
...
void step() {
  ...
  See `xformers.profiler.profile`
  */
  translation::unsupported_void("Translated placeholder for xformers/profiler/api.py::step");
}
