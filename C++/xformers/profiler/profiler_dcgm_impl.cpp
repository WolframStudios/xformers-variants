...
/*
Original Python source: xformers/profiler/profiler_dcgm_impl.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
class DCGMProfiler {
 public:
  ...
  Profiler that triggers start of DCGM profiler.
  */
  DCGMProfiler() = default;
  void __init__('_Profiler' main_profiler, std::optional<std::tuple<int64_t, ...>> gpus_to_profile = std::nullopt, std::any field_ids_to_profile, int64_t updateFreq = 5000) {
    /*
    Args:
        main_profiler: The main profiler object.
        gpus_to_profile: A tuple of integers representing the GPUs to profile. If `None`,
        ...
        updateFreq: The interval of two consecutive updates of each field. Defaults to 5000 microseconds.
            ...
            If the step to profile takes more than 5000 microseconds, then a larger updateFreq could also be used.
    */
    translation::unsupported_void("Translated placeholder for xformers/profiler/profiler_dcgm_impl.py::__init__");
  }
  std::optional<DcgmGroup> create_dcgm_group(Union[std::tuple<int64_t], Tuple[int64_t, ...]> gpus_to_profile) {
    return translation::unsupported<std::optional<DcgmGroup>>("Translated placeholder for xformers/profiler/profiler_dcgm_impl.py::create_dcgm_group");
  }
  Set[int64_t] get_profilable_fields() {
    return translation::unsupported<Set[int64_t]>("Translated placeholder for xformers/profiler/profiler_dcgm_impl.py::get_profilable_fields");
  }
  std::optional<DcgmFieldGroup> create_profiling_field_group(std::optional<std::tuple<int64_t, ...>> fieldIdsToProfile) {
    return translation::unsupported<std::optional<DcgmFieldGroup>>("Translated placeholder for xformers/profiler/profiler_dcgm_impl.py::create_profiling_field_group");
  }
  void __enter__() {
    translation::unsupported_void("Translated placeholder for xformers/profiler/profiler_dcgm_impl.py::__enter__");
  }
  void __exit__(std::any exc_type, std::any exc_val, std::any exc_tb) {
    translation::unsupported_void("Translated placeholder for xformers/profiler/profiler_dcgm_impl.py::__exit__");
  }
  void step() {
    translation::unsupported_void("Translated placeholder for xformers/profiler/profiler_dcgm_impl.py::step");
  }
};
