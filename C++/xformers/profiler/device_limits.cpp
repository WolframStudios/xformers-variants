...
/*
Original Python source: xformers/profiler/device_limits.py
# Copyright (c) Facebook, Inc. and its affiliates. All rights reserved.
#
# This source code is licensed under the BSD license found in the
# LICENSE file in the root directory of this source tree.
...
struct DeviceLimit {
 public:
  std::string name;
  std::string source;
  std::tuple<int64_t, int64_t> sm;
  double gmem_bandwidth;
  Mapping[torch.dtype, double] gemm_tflops;
  DeviceLimit() = default;
};

std::optional<DeviceLimit> get_device_limits(std::any device) {
  /*
  Currently only implemented for GPUs
  */
  return translation::unsupported<std::optional<DeviceLimit>>("Translated placeholder for xformers/profiler/device_limits.py::get_device_limits");
}
